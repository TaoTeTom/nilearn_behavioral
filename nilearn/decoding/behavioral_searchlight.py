...
def search_light(
    ...
    behavioral_data : array-like , optional
        Array containing additional behavioral data to include in the searchlight analysis.
    ...
    with warnings.catch_warnings():  # might not converge
        ...
        scores = Parallel(n_jobs=n_jobs, verbose=verbose)(
            delayed(_group_iter_search_light)(
                ...
                A.shape[0],
                verbose,
                behavioral_data=behavioral_data  # Pass behavioral data here
            )
...
def _group_iter_search_light(
    ...
    behavioral_data: array-like data to add to voxel fit
    ...
    t0 = time.time()
    for i, row in enumerate(list_rows): #for ball of radius x around voxel i
        kwargs = {"scoring": scoring, "groups": groups}
        X_row = X[:, row]
        if behavioral_data is not None: # adds behavioral data to voxel ball
            X_row = np.concatenate((X_row, behavioral_data), axis=1)
        par_scores[i] = np.mean(
            cross_val_score(estimator, X_row, y, cv=cv, n_jobs=1, **kwargs)
        )
        if verbose > 0:
...
class SearchLight(BaseEstimator):
    ...
    def fit(self, imgs, y, behavioral_data=None, groups=None):
        """Fit the searchlight.
        ...
        behavioral_data : array, optional
            Dictionary containing additional behavioral data to be included in the searchlight analysis. Keys should be behavioral feature names and values should be array-like of shape (n_samples,).
        ...
        # Concatenate behavioral data with fMRI data if provided
        if behavioral_data is not None:
            behavioral_data_array = np.column_stack([behavioral_data[key] for key in behavioral_data])
            # Expand dimensions of behavioral_data to match the 3D fMRI data structure
            behavioral_data_expanded = np.repeat(behavioral_data_array[:, np.newaxis, :], X.shape[1], axis=1)
            # Concatenate along the second axis (features axis)
            X = np.concatenate((X, behavioral_data_expanded), axis=2)
